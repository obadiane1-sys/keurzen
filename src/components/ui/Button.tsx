...
const variantConfig: Record<ButtonVariant, { bg: string; text: string; border?: string }> = {
  primary: { bg: Colors.coral, text: Colors.textInverse },
  secondary: { bg: Colors.mint, text: Colors.navy },
  ghost: { bg: 'transparent', text: Colors.textPrimary },
  danger: { bg: Colors.error, text: Colors.textInverse },
  outline: { bg: 'transparent', text: Colors.coral, border: Colors.coral },
};
    ...
    >
      {isLoading ? (
        ...
        <View style={styles.content}>
          ...
          <Text
            style={{ fontSize: sc.fontSize, fontWeight: '600', color: vc.text }}
          >
            {label}
          </Text>
